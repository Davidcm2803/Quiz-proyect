...
export default function PresentGameMenu() {
  ...
  useEffect(() => {
    ...
    ws.current.onmessage = (e) => {
      ...
      console.log("WS EVENT:", data.event, "current phase:", phase);
  ...
  if (phase === "waiting")
    return (
      <div className="min-h-screen bg-[#F8FBF3] flex flex-col items-center justify-center gap-6 px-4">
        <h2 className="text-[#292726] text-3xl font-black">
          ¡Listo, {playerId}!
        </h2>
        ...
        <div className="flex gap-1.5 mt-2">
          {[0, 1, 2].map((i) => (
            <div
              key={i}
              className="w-2 h-2 bg-[#e21b3c] rounded-full animate-bounce"
  ...
  if (phase === "starting")
    return (
      <div className="min-h-screen bg-[#1a1a2e] flex items-center justify-center">
        <h2 className="text-white text-5xl font-black animate-pulse">
          ¡Prepárate!
        </h2>
      </div>
    );
    ...
    return (
      <div className="min-h-screen bg-[#1a1a2e] flex flex-col relative overflow-hidden">
        ...
        <div className="h-2 bg-white/10 relative z-10">
          <div
            className="h-full bg-[#e21b3c] transition-all duration-1000"
        ...
        <div className="flex-1 flex flex-col relative z-10">
          <div className="flex justify-center pt-4 pb-2">
            <div
              className={`text-5xl sm:text-6xl font-black tabular-nums ${countdown <= 5 ? "text-[#e21b3c] animate-pulse" : "text-white"}`}
            >
              {countdown}s
            </div>
          </div>
  ...
  if (phase === "showAnswer")
    return (
      <div className="min-h-screen bg-[#1a1a2e] flex flex-col items-center justify-center gap-5 px-4">
        ...
        <h2
          className={`text-4xl font-black ${result?.correct ? "text-green-400" : result?.partial ? "text-yellow-400" : "text-[#e21b3c]"}`}
        >
          {result?.correct
            ? "¡Correcto!"
            : result?.partial
  ...
  if (phase === "answered")
    return (
      <div className="min-h-screen bg-[#1a1a2e] flex flex-col items-center justify-center gap-4 px-4">
        ...
        <h2
          className={`text-3xl font-black ${result?.correct ? "text-green-400" : result?.partial ? "text-yellow-400" : "text-[#e21b3c]"}`}
        >
          {result?.correct
            ? "¡Correcto!"
            : result?.partial
